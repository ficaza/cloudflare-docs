...
- A submission has no file.
- A submission has been made with an incorrect file extension.
- A submission has been made without an .EML attachment.
- Email Security does not recognize your domain. 
- A policy has been enforced that causes an email to be allowed or blocked.
- A reclassification has been requested on an email which is under an incorrect category.
- A submission has changed email aliases. For example, when a user submits to a team, or when a team submits as a user.

To ensure your submission is valid:

- Ensure your submission has a file attached.
- Ensure the file attached has an .EML file extension.
